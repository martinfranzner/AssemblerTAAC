    ...
    (vector<unsigned int> &instructionVector, Cache &cache, Memory &memory, unsigned int &initMemPos)
{
  ...
  int nextInstruction;
  while (status){
      nextInstruction = cache.consultCache(memory, PC);
    switch (nextInstruction){
      ...
      case 2: //MOV REG, REG
        ...
        //reg1 = cacheLines.consultCache(memory, PC);
      ...
      case 3: // MOV REG, MEM
        ...
        //reg1 = cacheLines.consultCache(memory, (PC));
        mem =  cache.consultCache(memory, (PC+1));
      ...
      case 4: // MOV MEM, REG
        PC++;
        reg1 = cache.consultCache(memory, PC +1);
      ...
      case 5: // MOV REG , IMEDIATE
        PC++;
        imediate = cache.consultCache(memory, PC +1);
      ...
      case 6: // MOV MEM, IMEDIATE
        PC++;
        mem = cache.consultCache(memory,PC);
        imediate = cache.consultCache(memory, PC +1);
      ...
      case 10: // JMP
        ...
        imediate = cache.consultCache(memory, (PC));
        PC = imediate;
            break;
      case 11: // JZ jump if zero
        PC++;
        if (FL == 0) {
          PC = PC + cache.consultCache(memory, (PC));
        } else {
          PC++;
        }
        break;
      ...
      case 14: // OUT REG
        ...
        if (cache.consultCache(memory, (PC)) == 0) { // OUT AX: ...
          cout << "AX " << regVet[0] << endl;
        } else if (cache.consultCache(memory, (PC)) == 1) { // OUT BX: ...
          cout << "BX " << regVet[1] << endl;
        } else if (cache.consultCache(memory, (PC)) == 2) { // OUT CX: ...
          cout << "CX " << regVet[2] << endl;
        } else if (cache.consultCache(memory, (PC)) == 3) { // OUT CX: ...
