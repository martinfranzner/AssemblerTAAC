...
void Fetcher::fetchInstructionInMemory(Memory &memory, int &posicaoIncialMemoria) {
...
void
Fetcher::fetchInstruction(vector<int> &instructionVector, Cache& cache, Memory &memory, int &posicaoIncialMemoria) {
  this->PC = static_cast<unsigned int>(posicaoIncialMemoria);
  bool status = true;
  int nextInstruction = -1;
  while (status) {
    if(!isInCache(instructionVector.at(PC), cache)){
      insertInstructionInCache(instructionVector.at(PC), memory, cache);
    }
    nextInstruction = this->fetchInCache(instructionVector.at(PC),cache).second;
...
bool Fetcher::isInCache(int instruction, Cache &cache) {
  for (int i=0; i<cache.getCacheLines()->size();i++) {
    if(cache.getCacheLines()->at(i).getWord() == instruction){
      return true;
    }
  }
  return false;
...
const pair<bool, int> Fetcher::fetchInCache(int instruction, Cache cache) const {
  for (int i=0; i<cache.getCacheLines()->size();i++) {
    if(cache.getCacheLines()->at(i).getWord() == instruction){
      return {true, cache.getCacheLines()->at(i).getWord()};
    }
  }
  return {false,-1};
}

void Fetcher::insertInstructionInCache(int &instruction, Memory &memory, Cache &cache) {
  for (int i=0; i<memory.getMemoria()->size();i++) {
    if(memory.getMemoria()->at(i) == instruction){
      CacheLine coco;
      coco.setWord(memory.getMemoria()->at(i));
      coco.setTag(static_cast<int>((i/memory.getMemoria()->size())*2048));
      coco.setRoll(-1);
      cache.insertInstrucion(coco);
      break;
    }
  }
}
